...
void handleServerTick (int64_t time_since_last_tick) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i ++) {
    ...
    uint8_t is_nearby_cactus = 0;
    is_nearby_cactus |= (getBlockAt(player->x + 1, player->y, player->z) == B_cactus);
    is_nearby_cactus |= (getBlockAt(player->x - 1, player->y, player->z) == B_cactus);
    is_nearby_cactus |= (getBlockAt(player->x, player->y, player->z + 1) == B_cactus);
    is_nearby_cactus |= (getBlockAt(player->x, player->y, player->z - 1) == B_cactus);

    // Tick damage from a cactus block if one is under/inside or around the player.
    if (block == B_cactus || is_nearby_cactus > 0) {
